    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // convert to car coordinates, as recommended in project Q&A video:
          ...
          // calculate polynomial coefficients from waypoints:
          double* ptr_x = &ptsx[0];
          double* ptr_y = &ptsy[0];
          ...
          // at this point, car is at the origin (everything is transformed into vehicle coordinate frame)
          // cte calculated at px=0: 
          double cte = polyeval(coeffs, 0);

          // calculate psi error at px=0, py=0, psi=0 (coordinate frame rotated)
          double epsi = -atan(coeffs[1]); // simplification from project Q&A video
          ...
          */
          ...
          // will set these according to N and dt in MPC!
          double poly_inc = 100/ 3.6/ 10.0;  //for a car going at 100km/h, put points every 100ms
          double num_points = 20; // 2s horizon for now....
